...
def route_random(datacenters, num_requests):
    ...
    if total < num_requests:
        first_dc = list(distribution.keys())[0]
        distribution[first_dc] += num_requests - total
...
def route_energy_only(datacenters, weather_data, cooling_selections, num_requests, ai_models=None):
    ...
    for dc_name, dc_info in datacenters.items():
        ...
        if ai_models and ai_models.is_trained:
            ...
            energy = ai_models.predict([temp, humidity, weather.get('wind_speed', 5), cooling_idx])
        else:
            energy = calculate_energy_per_request(temp, humidity, cooling)
...
def route_uhi_aware(datacenters, weather_data, cooling_selections, num_requests, ai_models=None):
    ...
    for dc_name, dc_info in datacenters.items():
        ...
        # UHI vulnerability factor: hot locations are more vulnerable
        uhi_vulnerability = 1 + 0.05 * max(0, temp - 25)
        
        # Wind mitigation: higher wind helps dissipate heat
        wind_benefit = 1 / (1 + 0.05 * wind)
...
def route_multi_objective(datacenters, weather_data, cooling_selections, user_location, 
    ...
    for dc_name, dc_info in datacenters.items():
        ...
        if ai_models and ai_models.is_trained:
            ...
            energy = calculate_energy_per_request(temp, humidity, cooling)
        energy_norm = energy / 1.0  # Normalize by ~1 Wh max
        
        # 2. Latency (normalized 0-1)
        distance = haversine_distance(user_lat, user_lon, dc_info['lat'], dc_info['lon'])
        latency = calculate_latency(distance)
        latency_norm = latency / 200  # Normalize by 200ms
        
        # 3. Carbon intensity (normalized 0-1)
        region = dc_info.get('region', 'default')
        carbon = get_carbon_intensity(region, hour)
        carbon_norm = carbon / 0.7  # Normalize by max ~0.7
        
        # 4. UHI vulnerability (normalized 0-1)
        uhi_score = max(0, temp - 15) / 30  # Higher temp = higher UHI risk
